...
ens_clf = VotingClassifier(list(enumerate(all_clfs)), voting='soft')

data = []
class_scores = []
metrics = []
start = datetime.datetime.now()
classifier = OneVsRestClassifier(ens_clf)
data.append(get_scores(classifier, X_t_train, y_train, X_t_test,y_test))
print (datetime.datetime.now() - start)
print ('-'*80)

data[0]['Classifier'] = 'Voting Ensemble'

o_v2_res = pd.DataFrame(data)
o_v2_res.to_excel("../data/o_votingModel_results.xlsx")
